...
def mnist_noniid(dataset, num_users):
    ...
    labels = dataset.train_labels.numpy()  # PyTorch dataset 특성에 따라 다를 수 있음
...
def mnist_noniid_unequal(dataset, num_users):
    """
    Sample non-I.I.D client data from MNIST dataset with unequal data amounts
    """
...
def cifar_noniid(dataset, num_users):
    ...
    dict_users = {i: np.array([]) for i in range(num_users)}
    idxs = np.arange(num_shards * num_imgs)
    labels = np.array(dataset.train_labels)  # CIFAR 데이터셋 속성에 따라 다름
...
if __name__ == '__main__':
    ...
    num_users = 100
    user_groups = mnist_noniid(dataset_train, num_users)
